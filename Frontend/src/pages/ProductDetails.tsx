...
import {
  ArrowLeft, Heart, Star, Minus, Plus,
  ChevronLeft, ChevronRight, ShieldCheck, Truck,
  Loader2, CheckCircle2, AlertCircle, XCircle, Trash2, Share2,
} from "lucide-react";
...
const ProductDetails = () => {
  ...
  useEffect(() => {
    ...
    productService
      ...
      .then(async (data) => {
        ...
        const inStockVariant = data.variants.find((v) => v.is_active && v.stock > 0);
        const defaultVariant = inStockVariant || data.variants.find((v) => v.is_active) || data.variants[0];
        if (defaultVariant) setSelectedVariant(defaultVariant);

        setSimilarLoading(true);
        try {
          let res = await productService.getAll({ category: data.category, limit: 10 });
          let candidates = res.products.filter((p) => p.id !== data.id);
          if (candidates.length < 2 && data.type) {
            res = await productService.getAll({ productType: data.type, limit: 10 });
  ...
  const selectByColorStorage = (color: string | null, storage: string | null) => {
    const match = activeVariants.find(
      (v) => (color === null || v.color === color) && (storage === null || v.storage === storage),
    );
    if (match) setSelectedVariant(match);
  };

  const handleColorSelect   = (color: string)   => selectByColorStorage(color, selectedStorage);
  const handleStorageSelect = (storage: string) => selectByColorStorage(selectedColor, storage);
  ...
  const handleSubmitReview = async () => {
    ...
    setSubmitError(null);
    try {
      await reviewService.create({ productId: product!.id, rating: newRating, comment: newComment.trim() || undefined });
      setNewRating(0);
      setNewComment("");
  ...
  const handleDeleteReview = async (reviewId: string) => {
    try {
      ...
      toast({ title: "Could not delete review", description: err.message, variant: "destructive" });
    }
  };
  ...
  const handleAddToCart = () => {
    if (!selectedVariant) { toast({ title: "Error", description: "Please select a variant first" }); return; }
    if (selectedVariant.stock <= 0) { toast({ title: "Out of Stock", description: "This variant is currently unavailable" }); return; }
    addToCart({
      id: product.id, name: product.name, price: selectedVariant.price, image: product.image,
      quantity, storage: selectedVariant.storage ?? undefined, color: selectedVariant.color,
      sku: selectedVariant.sku, variantId: selectedVariant.id,
    });
  ...
  const handleBuyNow = () => {
    if (!selectedVariant) { toast({ title: "Error", description: "Please select a variant first" }); return; }
    if (selectedVariant.stock <= 0) { toast({ title: "Out of Stock" }); return; }
    navigate("/checkout", {
      state: {
        buyNowItem: {
          id: product.id, name: product.name, price: selectedVariant.price, image: product.image,
  ...
  return (
    <div className="min-h-screen bg-white pb-24 sm:pb-0">
      ...
      <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-3">
        <Link to="/products" className="inline-flex items-center gap-1.5 text-sm text-gray-600 hover:text-gray-900 transition-colors">
      ...
      <div className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 pb-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10 xl:gap-14">
          ...
          <div className="relative">
            ...
            <div className="relative aspect-square bg-white rounded-2xl border border-gray-200 mb-3 overflow-hidden">
              ...
              />
              {/* Mobile wishlist & share buttons overlay */}
              ...
              <button
                onClick={() => setSelectedImage((p) => Math.min(galleryImages.length - 1, p + 1))}
              ...
              {galleryImages.map((img, i) => (
                <button
                  ...
                  className={`w-14 h-14 sm:w-16 sm:h-16 rounded-xl overflow-hidden border-2 transition-all bg-white flex-shrink-0 ${
                    selectedImage === i ? "border-purple-600" : "border-gray-200 hover:border-gray-300"
                  }`}
                >
          ...
          <div className="pt-1">
            ...
            {totalReviews > 0 && (
              <a href="#reviews" className="inline-flex items-center gap-2 mb-3 group">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              <div className="flex items-center gap-1.5 bg-green-50 border border-green-200 rounded-lg px-2.5 py-1.5">
                ...
                <span className="text-xs font-semibold text-green-800">Verified Authentic</span>
              </div>
              <div className="flex items-center gap-1.5 bg-blue-50 border border-blue-200 rounded-lg px-2.5 py-1.5">
                ...
                <span className="text-xs font-semibold text-blue-800">Safe Delivery</span>
              </div>
            </div>
            ...
            {activeVariants.length > 0 && (
              <div className="mb-4 space-y-3">
                {colors.length > 0 && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {colors.map((color) => {
                        const hasStock = activeVariants.some((v) => v.color === color && v.stock > 0);
                        return (
                          <button
                            key={color}
                ...
                {storages.length > 0 && (
                  <div>
                    <div className="text-xs font-semibold text-gray-600 mb-2">Storage</div>
                    <div className="flex flex-wrap gap-2">
                      {storages.map((s) => (
                        <button
                          key={s}
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="text-xs text-gray-500">
                      <span className="font-medium text-gray-800">
                        {[selectedVariant.color, selectedVariant.storage, selectedVariant.ram].filter(Boolean).join(" / ")}
                      </span>
                    </span>
            ...
            {/* Quantity */}
            <div className="mb-5">
              <div className="text-xs font-semibold text-gray-600 mb-2">Quantity</div>
            ...
            <p className="hidden sm:block text-xs text-gray-400 mt-2">Free inspection available before payment</p>
          </div>
        </div>

        {/* Key Highlights */}
        <div className="mt-10 pt-8 border-t border-gray-100">
          <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-5">Key Highlights</h2>
        ...
        {/* Reviews */}
        <div id="reviews" className="mt-10 pt-8 border-t border-gray-100 scroll-mt-20">
          <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-5">Reviews</h2>

          {totalReviews > 0 && (
            <div className="flex items-center gap-4 sm:gap-5 mb-6 bg-gray-50 rounded-2xl border border-gray-100 px-4 sm:px-6 py-4">
              <div className="text-4xl sm:text-5xl font-bold text-gray-900 leading-none">{averageRating.toFixed(1)}</div>
              <div>
          ...
          {!user ? (
            <div className="bg-gray-50 rounded-2xl border border-gray-100 px-5 py-7 mb-6 text-center">
              ...
              <p className="text-gray-700 font-semibold mb-1 text-sm">Have this product?</p>
              <p className="text-xs text-gray-500 mb-4">Sign in to share your experience.</p>
              <Link to="/login" className="inline-flex items-center gap-1 text-sm font-semibold text-purple-600 hover:text-purple-700 transition-colors">
            ...
            <div className="flex items-center gap-2 bg-purple-50 border border-purple-100 rounded-xl px-4 py-3 mb-6">
              ...
              <p className="text-sm text-purple-700 font-medium">You've already submitted a review.</p>
            </div>
          ) : (
            <div className="bg-gray-50 rounded-2xl border border-gray-100 px-4 sm:px-6 py-6 mb-6">
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((s) => (
                    ...
                    >
                      <Star
                        className={`w-8 h-8 sm:w-9 sm:h-9 transition-colors cursor-pointer ${
              ...
              <div className="mb-4">
                <p className="text-xs text-gray-600 mb-2">Your review (optional)</p>
                <textarea
                  value={newComment}
                  onChange={(e) => setNewComment(e.target.value)}
                  placeholder="Share your experience — build quality, performance, delivery, etc."
                  rows={4}
          ...
          {reviewsLoading ? (
            ...
            <div className="divide-y divide-gray-100">
              {reviews.map((review) => {
                const canDelete =
                  user &&
                  (String((user as any)._id) === review.user._id || (user as any).role === "admin");
                return (
                  <div key={review._id} className="py-5">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h4 className="font-bold text-gray-900 text-sm">{review.user.name || review.user.username}</h4>
        ...
        <div className="mt-10 pt-8 border-t border-gray-100">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-base sm:text-xl font-bold text-gray-900">Similar items</h2>
            <Link to="/products" className="text-sm text-purple-600 hover:text-purple-700 font-medium">View all →</Link>
          </div>
                  ...
                  <Link key={p.id} to={`/products/${p.slug}`} className="group block">
                    <div className="bg-gray-50 rounded-xl p-3 border border-gray-100 hover:border-purple-200 hover:shadow-md transition-all">
                      <div className="relative aspect-square mb-2 bg-white rounded-lg overflow-hidden">
                        ...
                        >
                          <Heart className={`w-3 h-3 ${inWishlistSimilar ? "fill-pink-500 text-pink-500" : "text-gray-400"}`} />
                        </button>
                      </div>
                      <h3 className="text-xs font-semibold text-gray-900 line-clamp-2 mb-1 leading-snug">{p.name}</h3>
                      <p className="text-xs font-bold text-purple-600">{formatPrice(p.price)}</p>
                    </div>
                  </Link>
                );
              })}
        ...
        <div className="px-4 py-3 safe-area-pb">
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-2 flex-1 ml-2">
              <button
                onClick={handleToggleWishlist}
                className={`w-11 h-11 flex-shrink-0 rounded-xl border-2 flex items-center justify-center transition-all ${
                  inWishlist ? "border-red-200 bg-red-50" : "border-gray-200 bg-white"
                }`}
              >
