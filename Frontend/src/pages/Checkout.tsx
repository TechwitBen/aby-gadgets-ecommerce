...
const Checkout = () => {
  ...
  // Keep a ref to the Paystack redirect timeout so we can cancel it on unmount
  ...
  // Cancel any pending Paystack redirect on unmount
  useEffect(() => {
    return () => {
  ...
  // ── FIX: guard also checks showSuccess so clearing the cart never triggers
  //         the "empty cart" screen while the success modal is visible.
  ...
  const handleSubmit = async () => {
    ...
    const invalidItems = orderItems.filter(
      (item) => !item.variantId || item.quantity < 1,
    );
    ...
    try {
      ...
      // Store order details immediately — before anything else can fail
      setCreatedOrderId(order._id);
      setCreatedOrderNumber(order.order_number ?? "");

      if (paymentMethod === "pod") {
        // ── POD flow ──────────────────────────────────────────────────────
        // Show success FIRST, then clear cart.
        // If showSuccess is set before clearCart, both batch together in React 18
        // and the `orderItems.length === 0 && !showSuccess` guard stays false.
        setShowSuccess(true);
        if (!isBuyNow) clearCart();
      } else {
        // ── Online (Paystack) flow ────────────────────────────────────────
        try {
          const { authorization_url } = await paymentService.initializePayment({
            orderId: order._id,
          });

          // Show modal FIRST, clear cart second — same reason as POD
          setShowSuccess(true);
          if (!isBuyNow) clearCart();

          // Redirect after a short delay so the user sees the success screen
          paystackRedirectTimer.current = setTimeout(() => {
            window.location.href = authorization_url;
          }, 2500);
        } catch {
          // Order is saved — we just couldn't reach Paystack.
          // Show modal with the "payment pending" warning so user can retry from Orders.
      ...
      // Reaches here only if ORDER CREATION failed — nothing was saved.
      setSubmitError(
  ...
  return (
    <div className="min-h-screen bg-white">
            ...
            <section className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-6">
              ...
              >
                {deliveryEnabled && (
                  ...
                  >
                    <MapPin
                      className={`w-6 h-6 ${fulfillment === "delivery" ? "text-purple-600" : "text-gray-500"}`}
                    />
                    <span
                ...
                {pickupEnabled && (
                  ...
                  >
                    <Store
                      className={`w-6 h-6 ${fulfillment === "pickup" ? "text-purple-600" : "text-gray-500"}`}
                    />
                    <span
      ...
      <OrderSuccessModal
        ...
        onClose={() => {
          // Cancel any pending Paystack redirect if user manually closes the modal
          if (paystackRedirectTimer.current) {
            clearTimeout(paystackRedirectTimer.current);
          }
