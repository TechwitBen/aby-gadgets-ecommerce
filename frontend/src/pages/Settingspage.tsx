...
const AddressModal = ({
  ...
  open: boolean; address: AddressFormData | null;
  onClose: () => void; onSave: (data: AddressFormData) => Promise<void>;
}) => {
      ...
      <div className="bg-white rounded-t-3xl sm:rounded-2xl w-full sm:max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="px-5 py-5 space-y-4">
            ...
            <div className="flex gap-2 flex-wrap">
              {ADDRESS_LABELS.map((l) => (
                <button
                  ...
                  style={form.label === l ? { backgroundColor: BRAND, borderColor: BRAND, color: "#fff" } : {}}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all ${
                    form.label === l ? "" : "bg-gray-50 text-gray-600 border-gray-200 hover:border-gray-300"
                  }`}
                >
          ...
          <div>
            ...
            <Input value={form.street} onChange={set("street")} placeholder="e.g. 9, Adepele Street, Lekki"
          ...
          <div>
            ...
            <Input value={form.postal_code} onChange={set("postal_code")} placeholder="100001"
              className="h-11 rounded-xl border-gray-200 text-sm" />
          </div>
...
const UserSettingsPage = () => {
  ...
  const [prefs, setPrefs] = useState({
    orderUpdates: true, emailNotifications: true, paymentAlerts: true,
  });
  ...
  const handleAddrSave = async (data: AddressFormData) => {
    try {
      const payload = {
        ...
        postal_code: data.postal_code, isDefault: data.isDefault,
      };
  ...
  const initials = getInitials(profile?.name, profile?.username, profile?.email);

  const pwStrength =
    newPw.length === 0 ? 0 : newPw.length < 6 ? 1 : newPw.length < 8 ? 2 : newPw.length < 12 ? 3 : 4;
          ...
          <div className="flex overflow-x-auto no-scrollbar -mx-4 px-4 gap-0">
            {TABS.map(({ key, label, icon: Icon }) => (
              <button
                key={key}
                onClick={() => setTab(key)}
                style={tab === key ? { borderBottomColor: BRAND, color: BRAND } : {}}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="px-5 sm:px-6 py-5 space-y-4">
              ...
              >
                ...
                <div>
                  <p className="text-sm font-semibold" style={{ color: BRAND }}>Your Avatar</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input value={name} onChange={(e) => setName(e.target.value)}
                    placeholder="John Doe" className="h-11 rounded-xl border-gray-200 text-sm" />
                </div>
                <div>
        ...
        {tab === "addresses" && (
          <div className="space-y-4">
            <div
              ...
              style={{ backgroundColor: BRAND_LIGHT, border: `1px solid ${BRAND_BORDER}` }}
            >
              <AlertCircle className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: BRAND }} />
              <div>
                <p className="text-xs font-semibold" style={{ color: BRAND }}>Delivery name & phone</p>
                <p className="text-xs mt-0.5 leading-relaxed" style={{ color: "#7C55E8" }}>
                ...
                {addresses.map((addr) => (
                  <div
                    ...
                    style={addr.isDefault
                      ? { border: `1.5px solid ${BRAND}`, boxShadow: `0 0 0 3px ${BRAND_LIGHT}` }
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        ...
                        <button
                          onClick={() => setAddrModal({ open: true, address: { _id: addr._id, label: addr.label, street: addr.street, city: addr.city, state: addr.state, country: addr.country, postal_code: addr.postal_code, isDefault: addr.isDefault } })}
        ...
        {tab === "notifications" && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="px-5 sm:px-6 divide-y divide-gray-50">
              <Toggle checked={prefs.orderUpdates} onChange={(v) => handlePrefChange("orderUpdates", v)}
                label="Order Updates" description="Status changes: confirmed, shipped, out for delivery, delivered" />
              <Toggle checked={prefs.paymentAlerts} onChange={(v) => handlePrefChange("paymentAlerts", v)}
                label="Payment Alerts" description="Confirmations, failures, and refund updates" />
              <Toggle checked={prefs.emailNotifications} onChange={(v) => handlePrefChange("emailNotifications", v)}
                label="Email Notifications" description="Order confirmations and updates sent to your email" />
            </div>
