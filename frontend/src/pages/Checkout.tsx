...
const nigerianStates = [
  ...
  "Anambra", "Imo", "Abia", "Ebonyi", "Osun", "Ondo", "Ekiti", "Kwara",
  "Niger", "Nasarawa", "Plateau", "Kaduna", "Kebbi", "Zamfara", "Katsina",
...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    userService.getProfile().then((profile) => {
      ...
      if (profile.phone?.trim()) { setPhone(profile.phone.trim()); filled = true; }
      if (profile.email)         { setEmail(profile.email);         filled = true; }

      const defaultAddr =
        profile.addresses?.find((a) => a.isDefault) ?? profile.addresses?.[0];
      if (defaultAddr) {
        if (defaultAddr.street?.trim()) { setAddress(defaultAddr.street.trim()); filled = true; }
        if (defaultAddr.state?.trim())  { setState(defaultAddr.state.trim());    filled = true; }
      }
  ...
  const paymentOptions = [
    ...
    Icon: React.FC<{ className?: string }>; subtitle: string;
  }[];
  ...
  const validateField = (field: keyof FieldErrors, value: string) => {
    ...
    switch (field) {
      case "firstName":
        ...
        else if (!isValidNamePart(value))   error = "First name contains invalid characters.";
        break;
      case "lastName":
        if (!value.trim())                  error = "Last name is required.";
        else if (!isValidNamePart(value))   error = "Last name contains invalid characters.";
        break;
      case "phone":
      ...
      case "address":
        ...
        else if (value.trim().length < 5)   error = "Please enter a more complete address.";
        break;
      case "state":
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                ...
                <div>
                  ...
                  {fieldErrors.phone ? (
                    ...
                    <p className="text-xs text-gray-400 mt-1">Nigerian numbers only (e.g. 0801 234 5678)</p>
                  )}
                </div>
                ...
                <div className="space-y-4">
                    ...
                    {zones.length === 0 ? (
                      ...
                      <>
                        <Select value={selectedZone?.city ?? ""} onValueChange={handleZoneSelect}>
                          <SelectTrigger className={`h-10 text-sm rounded-xl ${fieldErrors.zone ? "border-red-400" : "border-gray-200"}`}>
                        ...
                        {fieldErrors.zone && (
                          <p className="text-xs text-red-500 mt-1">{fieldErrors.zone}</p>
                        )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    ...
                    <div>
                      ...
                      >
                        <SelectTrigger className={`h-10 text-sm rounded-xl ${fieldErrors.state ? "border-red-400" : "border-gray-200"}`}>
              ...
              {fulfillment === "pickup" && deliveryConfig && (
                <div className="bg-green-50 border border-green-200 rounded-xl p-4 space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="ml-auto text-xs font-semibold px-2 py-0.5 rounded-full bg-green-100 text-green-700">Free</span>
                  </div>
                  {deliveryConfig.pickupAddress && (
                    <div>
            ...
            <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-6">
              ...
              <div className={`grid gap-3 mb-4 ${paymentOptions.length === 1 ? "grid-cols-1 max-w-xs" : "grid-cols-2"}`}>
                {paymentOptions.map((m) => (
                  ...
                  >
                    <m.Icon className={`w-6 h-6 ${paymentMethod === m.value ? "text-purple-600" : "text-gray-400"}`} />
                    <span className={`text-xs font-semibold text-center leading-tight ${paymentMethod === m.value ? "text-purple-700" : "text-gray-700"}`}>
                      {m.label}
                    </span>
                    <span className={`text-[10px] font-medium ${paymentMethod === m.value ? "text-purple-500" : "text-gray-400"}`}>
                      {m.subtitle}
                    </span>
                  </button>
                ))}
              ...
              {paymentMethod === "online" && (
                <div className="rounded-2xl p-4 text-center bg-gradient-to-br from-purple-50 to-violet-50 border border-purple-100">
                  ...
                  <p className="text-xs text-gray-500">
                    After placing your order you'll be prompted to pay via Card, Bank Transfer, or USSD.
                  </p>
