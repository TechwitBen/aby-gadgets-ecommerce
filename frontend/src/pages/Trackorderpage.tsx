...
const PICKUP_STEPS: { label: string; description: string }[] = [
  ...
  {
    label: "Collected",
    description: "You have collected your order. Thank you!",
  },
];
        ...
        <div className="flex items-start gap-3 px-4 py-4 bg-red-50">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-red-700 mt-0.5 leading-relaxed">
              ...
              was declined by your bank or card issuer (insufficient funds,
              wrong OTP, card restrictions, or failed transfer). Your order is
              still saved — try a different card or payment method.
            </p>
            {retryError && (
        ...
        <div className="flex items-start gap-3 px-4 py-4 bg-amber-50">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-700 mt-0.5 leading-relaxed">
              You closed the Paystack payment page before completing your
              payment of <strong>₦{order.total.toLocaleString()}</strong>. Your
              order is still saved — complete payment to confirm it.
            </p>
            {retryError && (
              <p className="text-xs text-red-600 font-medium mt-2">
                {retryError}
        ...
        <div className="flex items-start gap-3 px-4 py-4 bg-orange-50">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-orange-700 mt-0.5 leading-relaxed">
              Payment was not initialised for this order — no payment record
              exists yet. This can happen if there was a network error during
              checkout. Tap below to start your payment of{" "}
      ...
      <div className="mb-6 flex items-start gap-3 px-4 py-4 bg-blue-50 border border-blue-200 rounded-2xl">
        ...
        <div>
          ...
          <p className="text-xs text-blue-700 mt-0.5 leading-relaxed">
            ...
            <strong>₦{order.total.toLocaleString()}</strong>. This usually
            resolves within a minute. Your order will confirm automatically.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="mb-6 flex items-start gap-3 px-4 py-4 bg-amber-50 border border-amber-200 rounded-2xl">
      ...
      <div className="flex-1">
        ...
        <p className="text-xs text-amber-700 mt-0.5 leading-relaxed">
          This order has not been paid yet. Complete payment to confirm your
          order.
        </p>
        {retryError && (
          <p className="text-xs text-red-600 font-medium mt-2">{retryError}</p>
        )}
        <button
          onClick={onRetry}
          disabled={isRetrying}
...
const OrderSummaryCard = ({
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
      ...
      <div className="mt-3">
        <span
          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border ${payBadge.cls}`}
        >
          ...
          {order.pickup_code && (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <Store
                  className={`w-3.5 h-3.5 ${isReadyForPickup ? "text-teal-600" : "text-gray-500"}`}
                />
                <p
                  className={`text-xs font-semibold ${isReadyForPickup ? "text-teal-600" : "text-gray-500"}`}
                >
                  Pickup Code
                </p>
              </div>
          ...
          {isCollected ? (
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-gray-500 text-sm mb-6">
                Your order has been collected from our store.
              </p>
              <button
                onClick={onBack}
                className="px-8 py-3 rounded-xl text-sm font-semibold text-white hover:opacity-90"
                style={{
              ...
              <div className="relative">
                {PICKUP_STEPS.map((step, i) => {
                  ...
                  return (
                    <div
                      key={step.label}
                      className={`relative flex gap-5 ${!last ? "pb-8" : ""}`}
                    >
                      {!last && (
                        <div
                          className="absolute left-5 top-10 w-0.5"
                          style={{
                            backgroundColor: done ? "#0d9488" : "#E5E7EB",
                      ...
                      >
                        {done ? (
                          ...
                          <span
                            className={`text-xs font-bold ${current ? "text-teal-600" : "text-gray-400"}`}
                          >
                            {i + 1}
                          </span>
                        )}
                      ...
                      <div className="flex-1 pt-1">
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          <p
                            className={`text-base font-semibold ${done || current ? "text-gray-900" : "text-gray-400"}`}
                          >
                            {step.label}
                          </p>
                          {current && (
                        ...
                        <p
                          className={`text-sm ${i > currentStep ? "text-gray-300" : "text-gray-600"}`}
                        >
                          {step.description}
                        </p>
                      </div>
...
const TrackOrderPage = () => {
  ...
  const handleRetry = async () => {
    ...
    try {
      ...
      setRetryError(
        res.message ?? "Could not open payment. Try from your orders page.",
      );
    } catch (err: any) {
      setRetryError(
                ...
                <div className="relative">
                  {DELIVERY_STEPS.map((step, i) => {
                    ...
                    return (
                      <div
                        key={step.label}
                        className={`relative flex gap-5 ${!last ? "pb-8" : ""} group`}
                      >
                        {!last && (
                          <div
                            className="absolute left-5 top-10 w-0.5 transition-all"
                            style={{
                              backgroundColor: done ? "#6426E1" : "#E5E7EB",
                              height: "calc(100% - 2rem)",
                            }}
                          />
                        )}
                        ...
                        <div className="flex-1 pt-1">
                          ...
                          <p
                            className={`text-sm leading-relaxed ${i > currentStep ? "text-gray-300" : "text-gray-600"}`}
                          >
                            {step.description}
                          </p>
                        </div>
